  ...
  export default {
    ...
    imprimirReceta(cita) {
      ...
      logo.onload = () => {
        ...
        doc.setFontSize(12);
        doc.text('Paciente:', 15, 70);
        doc.setFont('helvetica', 'normal');
        doc.text(`${cita.pacienteId?.nombre} ${cita.pacienteId?.apellidos}`, 40, 70);
        doc.text('Médico:', 15, 80);
        doc.setFont('helvetica', 'normal');
        doc.text(`${cita.medicoId?.nombre} ${cita.medicoId?.apellidos}`, 40, 80);
        doc.setFont('helvetica', 'bold');
        doc.text('Fecha:', 15, 90);
        doc.setFont('helvetica', 'normal');
        doc.text(`${this.formatFecha(cita.fecha)}`, 40, 90);
        doc.setFont('helvetica', 'bold');
        doc.text('Hora:', 15, 100);
        doc.setFont('helvetica', 'normal');
        doc.text(`${cita.hora}`, 40, 100);
        doc.setFont('helvetica', 'bold');
        doc.text('Especialidad:', 15, 110);
        doc.setFont('helvetica', 'normal');
        doc.text(`${cita.especialidadId?.nombre}`, 40, 110);
        doc.setFont('helvetica', 'bold');
        doc.text('Prestación:', 15, 120);
        doc.setFont('helvetica', 'normal');
        doc.text(`${cita.prestacionId?.nombre}`, 40, 120);
    ...
    async descargarReceta(cita) {
      ...
      logo.onload = () => {
        ...
        // Descargar el PDF
        doc.save(`Justificante_${this.formatFecha(cita.fecha)}.pdf`);
      };
    }
  };
</script>
